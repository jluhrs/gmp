...
/**
 * This is a copy of Sun's LoaderHandler with a single change: the codebase isn't cached.
 * @see sun.rmi.server.LoaderHandler
 */
@SuppressWarnings({ "deprecation", "unchecked" })
public final class NonCachingLoaderHandler {
	...
	Log.getLog("sun.rmi.loader", "loader", NonCachingLoaderHandler.logLevel);
...
//    }
     ...
     */
    private NonCachingLoaderHandler() {}
	...
	if (codebaseURLs == null) {
		String codebaseProperty = getCodebaseProperty();
	...
	if (annotation != null) {
	    ...
	    return getCodebaseProperty();	// REMIND: does this make sense??
	}
    }
	...
	synchronized (NonCachingLoaderHandler.class) {
	    /*
    ...
    /**
     * Add the permissions necessary to load classes from a loader with
     * the given URLs for a codebase to the given permissions object.
     */
    ...
    {
	/*
	 * The approach used here was derived from the method
	 * getAccessControlContext() in the sun.applet.AppletPanel class.
	 */
	for (int i = 0; i < urls.length; i++) {
	    URL url = urls[i];
	    ...
	    try {
		/*
		 * Get the permission required to connect to the particular
		 * codebase URL, according to the URL's connection object.
		 */
		...
		if (p != null) {
			...
			/*
			 * Otherwise, assume that the permission we have is
			 * the real permission required to download classes
			 * from beneath the codebase URL.
			 */
			perms.add(p);
			/*
