...
case class LogEventWrapper(level0: PaxLevel, timeStamp0: Long, message0: String, loggerName0: String) {

  def this(event: PaxLoggingEvent) {
    this (event.getLevel, event.getTimeStamp, event.getMessage, event.getLoggerName)
  }

  @BeanProperty val level = level0
