...
class Activator extends BundleActivator {
  ...
  def start(context: BundleContext) {
    ...
    factoryTracker = track[ActionMessageBuilder, CommandHandlers, SequenceCommandExecutorFactory](context) { (b, ch) =>
        val f = SequenceCommandExecutorFactory(b, am, ch, context)
...
case class SequenceCommandExecutorFactory(builder: ActionMessageBuilder, manager: ActionManager, handler: CommandHandlers, context: BundleContext) extends ManagedServiceFactory {
  ...
  private def registerExecutor(pid: String, startup: String) = {
    deleted(pid)
    val executor = new SequenceCommandExecutorStrategy(builder, manager, handler, startup)
    try {
