...
class GDSConfigurationParserSpec extends Spec with ShouldMatchers {
    describe("A GDSConfigurationParser") {
        it("should parse configuration line") {
            ...
            result.get(0) should equal (GDSConfiguration(Instrument("GPI"), ObservationEvent("OBS_END_ACQ"), Keyword("AIRMASS"), DataType("DOUBLE"), Mandatory(false), NullValue("NONE"), Subsystem("EPICS"), Channel("ws:massAirmass"), ArrayIndex("NULL"), FitsComment("Mean airmass for the observation")))
        }
