 ...
 *
 * Whenever an action in the client code is finished, it has to report
 * back to the GMP with their completion information. Completion information
 * can be either COMPLETED or ERROR.
 * 
 *
 */
public class JMSCompletionInfoConsumer implements MessageListener, ExceptionListener {

    private static final Logger LOG = Logger.getLogger(JMSCompletionInfoConsumer.class.getName());


    public JMSCompletionInfoConsumer() {
        ...
        try {
            ...
            Destination destination = session.createQueue(GMPKeys.GMP_COMPLETION_INFO);
            MessageConsumer consumer = session.createConsumer(destination);
            consumer.setMessageListener(this);
            LOG.info("Message Listener started to receive completion information");
    ...
    public void onMessage(Message message) {
        ...
        try {
            if (message instanceof MapMessage) {
                ...
                LOG.info("Received Completion info for action ID " + actionId + " : " + response.toString());
