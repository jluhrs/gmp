...
class GDSConfigurationParserSpec extends Spec with ShouldMatchers {
    ...
    def verifyParsedLine(text: String): Any = {
        ...
        result.get should have length (1)
        result.get(0) should equal(GDSConfiguration(Instrument("GPI"), GDSEvent("OBS_END_ACQ"), new FitsKeyword("AIRMASS"), HeaderIndex(0), DataType("DOUBLE"), Mandatory(false), DefaultValue("NONE"), Subsystem(KeywordSource.EPICS), Channel("ws:massAirmass"), ArrayIndex(0), FitsComment("Mean airmass for the observation")))
    }
